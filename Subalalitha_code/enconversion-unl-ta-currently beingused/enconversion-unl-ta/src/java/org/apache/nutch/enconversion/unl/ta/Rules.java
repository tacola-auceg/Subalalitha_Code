...
public class Rules {
		...
		try {
			...
			while (j < total) {
				...
				if (cindex >= 0) {
					...
					if (bnode.next != null) {
						te = disambiguate(rword, j);
						// te = wsd.processWSD(String sword,int ind);
	...
	public ArrayList<String> disambiguate(String sword, int j) {
		String getSense = "";
		try {
			while (bn != null) {
				// rootlist.add(bn.lexeme+"/"+bn.headword+"/"+bn.constraint);
			...
			sense = RulesSet(j, sword);
			// getSense = wsd.processWSD(rootlist,anal,pos,sword,j);
			// System.out.println("getSense:"+getSense);
		} catch (Exception e) {
	...
	public ArrayList<String> RulesSet(int j, String sword) {
